...
class BrowseApp(App[object]):
    ...
    def __init__(self, cfg: Config, client: JellyfinClient, items: list[MediaItem], theme_name: str):
        ...
        self.info_scroll = 0
        self.info_item: Optional[MediaItem] = None
        self.sort_desc = True
    ...
    def on_key(self, event) -> None:
        ...
        if self.info_visible:
            ...
            if event.key in {"q", "escape", "f2"} or typed == "q":
                self.info_visible = False
                self.render_items()
                event.stop()
                return
    ...
    def render_info(self) -> None:
        ...
        info_text.append("q close | Enter play | ←/→ episode | [/] season | ↑/↓ scroll", style="dim")
    ...
    def render_help(self) -> None:
        ...
        help_text.append("F2           show selected item info\n")
        help_text.append("Info: q close, Enter play, ←/→ episode, [/] season\n")
