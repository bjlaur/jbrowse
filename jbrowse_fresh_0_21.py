...
class BrowseApp(App[object]):
    ...
    def on_key(self, event) -> None:
        ...
        if self.info_visible:
            if event.key in {"q", "escape", "ctrl+i"}:
                self.info_visible = False
                self.render_items()
                event.stop()
                return
    ...
    def render_info(self) -> None:
        ...
        if not self.visible_items:
            ...
            lines = self.visible_items[self.selected_index].info_lines
        ...
        info_text.append("\n\n")
        info_text.append("q close | ↑/↓ scroll | PgUp/PgDn | Home/End", style="dim")
        self.listbox.update(self.overlay_panel(info_text, "Info"))
    ...
    def render_help(self) -> None:
        ...
        help_text.append("Ctrl+I       show selected item info\n")
    ...
    def update_status(self) -> None:
        ...
        sort_arrow = "desc" if self.sort_desc else "asc"

        parts = [
            f"sort: {sort_label} ({sort_arrow})",
