...
class BrowseApp(App[object]):
    ...
    def __init__(self, cfg: Config, items: list[MediaItem], theme_name: str):
        super().__init__()
        self.cfg = cfg
    ...
    def render_items(self) -> None:
        ...
        for index in range(start, end):
            title = self.visible_items[index].title
...
def browser_loop(cfg: Config, client: JellyfinClient, items: list[MediaItem], themes: list[Theme]) -> int:
    ...
    while True:
        ...
        chosen = BrowseApp(cfg, items, theme.name).run()
