...
| 10 | Press `y` on replace — new item plays, old session stops | [ ] | [x] | Can't simulate IPC start in harness | Pass. |
...
| 15 | Info page Progress auto-updates without cursor movement | [ ] | [ ] | Timer-based screen refresh can't be verified in static SVG | **FAIL** — needs cursor movement. **FIX APPLIED**: added self.refresh() in poll. |
...
| # | Test | Harness | Manual | Why no harness? | Notes |
|---|------|---------|--------|-----------------|-------|
| 21 | Press Enter on replace prompt → starts playback | [ ] | [ ] | Harness can't verify playback actually started (no real IPC) | **FIX APPLIED**: Enter now mapped to play. |
...
| 24 | Press `y` on replace → new item plays, old session stops | [ ] | [ ] | Can't simulate IPC start in harness | |
...
| 27 | Info page Progress auto-updates without cursor movement | [ ] | [ ] | Timer-based screen refresh can't be verified in static SVG | **FIX APPLIED**: added self.refresh() in poll. |
...
| 36 | Press `Ctrl+B` → video does NOT restart (position preserved) | [ ] | [ ] | Can't verify seamless transition in harness (no real mpv) | **FIX APPLIED**: seek back after loadfile_replace. |
...
**Fixed, needs re-test:** 4, 7, 8, 11, 16, 21, 22, 27, 28, 29, 30, 36

**Still failing:** 5 (Ctrl+P Textual palette), 15/27 (info Progress auto-update)

**Harness-only (static SVG can't verify):** 10, 15, 21, 24, 27, 36 — need manual testing
