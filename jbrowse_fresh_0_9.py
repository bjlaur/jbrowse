...
def create_blank_cfg(path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("", encoding="utf-8")

    die(
        f"Created blank config at {path}\n\n"
        "Edit it, then run jbrowse again. Minimal contents:\n\n"
        "[jellyfin]\n"
...
def load_cfg(path: Path) -> Config:
    ...
    return Config(
        ...
        max_display_items=max(0, parser.getint("ui", "max_display_items", fallback=0)),
    )
...
def sort_for_view(items: list[MediaItem], view: str) -> list[MediaItem]:
    if view == "played":
        played = [item for item in items if item.last_played]
        return sorted(played, key=lambda item: parse_jf_date(item.last_played), reverse=True)

    return sorted(items, key=lambda item: parse_jf_date(item.date_created), reverse=True)
...
class BrowseApp(App[Optional[MediaItem]]):
    ...
    BINDINGS = [
        ("tab", "toggle_view", "toggle played/added"),
    ...
    def apply_filter(self) -> None:
        ...
        source = sort_for_view(self.all_items, self.view)
    ...
    def update_status(self) -> None:
        parts = [
            self.view,
            f"{len(self.all_items)} loaded",
            f"{len(self.filtered_items)} matched",
        ]

        if len(self.visible_items) != len(self.filtered_items):
            parts.append(f"showing {len(self.visible_items)}")

        if self.regex_error:
            parts.append(f"regex error: {self.regex_error}")

        parts.append("tab toggle")
