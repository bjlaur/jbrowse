...
Style:
  1. jbrowse.tcss next to this script, if it exists
  2. ~/.config/jbrowse/jbrowse.tcss, if it exists
  3. built-in fallback
...
def load_tcss(path: Optional[Path]) -> tuple[str, str]:
    if path is None:
        return DEFAULT_TCSS, "built-in"
...
def browser_loop(cfg: Config, client: JellyfinClient, items: list[MediaItem], tcss: str) -> int:
    BrowseApp.CSS = tcss
...
def main() -> int:
    ...
    style_path = Path(args.style).expanduser() if args.style else default_style_path()
    ...
    if args.print_style_path:
        print(style_path if style_path is not None else "built-in")
        return 0

    tcss, style_name = load_tcss(style_path)
    ...
    print(f"Using style: {style_name}", file=sys.stderr)
    ...
    return browser_loop(cfg, client, items, tcss)
